...
exports.getRandomPhone = async (req, res) => {
  ...
  try {
    ...
    try {
      const [countResults] = await connection.query('SELECT COUNT(*) AS count FROM phones WHERE is_taken = FALSE');
      ...
      const [randomResults] = await connection.query('SELECT * FROM phones WHERE is_taken = FALSE LIMIT 1 OFFSET ?', [randomIndex]);
      const phone = randomResults[0];

      await connection.query('UPDATE phones SET is_taken = TRUE WHERE id = ?', [phone.id]);
      ...
      res.status(500).send(`Error during transaction: ${err.message}`);
    } finally {
      connection.release();
    }
  } catch (err) {
